...
SELECT ( case when ifnull(c.cnt,0) = 0 then 999999
  ...
  FROM 
   (SELECT  valuestring AS cnt,rec_date
     ...
     AND key_val ='umts_rssqry_cell_cnt'
     UNION ALL
     SELECT  count(*) AS cnt,rec_date
     FROM s_info_umts_stg 
     GROUP BY rec_date
     HAVING COUNT(*) <> 0
...
SET @latest_cnt = (SELECT count(*) AS latest_cnt FROM s_info_umts_stg);
SET @cnt_list  = (SELECT group_concat(concat('[ ',date(rec_date) ,' - ', cnt,' ]')) from (SELECT  count(*) AS cnt,rec_date  FROM s_info_umts WHERE rec_date < @p_date GROUP BY rec_date ORDER BY rec_date DESC LIMIT 5) AS A);
